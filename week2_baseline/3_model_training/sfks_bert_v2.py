# -*- coding: utf-8 -*-
"""
CAIL2020 司法考试 BERT + LoRA 微调脚本 (优化版)
改进点:
1. 只使用单选题训练（过滤多选题）
2. 增加训练数据量
3. 优化超参数
...
OUTPUT_DIR = SCRIPT_DIR.parent / "models" / "sfks_bert_lora_output_v2"
MAX_SEQ_LENGTH = 512

# LoRA 参数（优化）
LORA_RANK = 32          # 增加秩
LORA_ALPHA = 64         # 对应调整 alpha
LORA_DROPOUT = 0.1      # 增加 dropout 防止过拟合

# 训练参数（优化）
SAMPLE_SIZE = None      # 使用全部单选题数据
TEST_SPLIT = 0.1
ONLY_SINGLE_CHOICE = True  # 只使用单选题

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def load_sfks_data(data_dir: Path, sample_size: int = None, only_single_choice: bool = True):
    """
    加载司法考试数据
            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        # 判断是单选还是多选
                        is_single = isinstance(answer, list) and len(answer) == 1

                        if only_single_choice and not is_single:
                        ...
                        if isinstance(processed["option_list"], dict) and len(processed["option_list"]) == 4:
                            data_list.append(processed)
                    except json.JSONDecodeError:
                        continue

    print(f"  ✓ 单选题: {single_choice_count} 条")
    print(f"  ✓ 多选题(已过滤): {multi_choice_count} 条")
    print(f"  ✓ 有效数据: {len(data_list)} 条")

    # 检查答案分布
    answer_dist = {"A": 0, "B": 0, "C": 0, "D": 0}
    for item in data_list:
        ans = item["answer"][0] if item["answer"] else "A"
...
if __name__ == "__main__":
    print("=" * 60)
    print("CAIL2020 司法考试 BERT + LoRA 微调 (优化版)")
    print("=" * 60)
    ...
    dataset = load_sfks_data(DATA_DIR, sample_size=SAMPLE_SIZE, only_single_choice=ONLY_SINGLE_CHOICE)
    ...
    lora_config = LoraConfig(
        ...
        bias="none",
        target_modules=["query", "key", "value", "dense"],  # 扩展目标模块
    )
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=16, # 增加批大小
        per_device_eval_batch_size=16,
        num_train_epochs=15,            # 增加轮数
